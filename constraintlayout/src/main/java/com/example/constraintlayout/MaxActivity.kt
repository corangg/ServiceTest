...
class MaxActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            Log.d("ConstraintLayoutDrawTime", "ConstraintLayout draw Fail: $index")
        }

        val endTime = System.nanoTime()
        val duration = endTime - startTime
        Log.d("ConstraintLayoutDrawTime", "Max ConstraintLayout create time: $duration ns")
        val createTimeTextView = TextView(this).apply {
            id = View.generateViewId()
            text = "Max ConstraintLayout create time: $duration ns"
            layoutParams = ConstraintLayout.LayoutParams(
        ...
        rootLayout.viewTreeObserver.addOnPreDrawListener(object :
            ...
            override fun onPreDraw(): Boolean {
                ...
                Log.d(
                    ...
                    "Max ConstraintLayout draw time: $totalDuration ns"
                )

                // 화면에 표시되는 시간 텍스트뷰 추가
                val drawTimeTextView = TextView(this@MaxActivity).apply {
                    id = View.generateViewId()
                    text = "Max ConstraintLayout draw time: $totalDuration ns"
                    layoutParams = ConstraintLayout.LayoutParams(
                        ConstraintLayout.LayoutParams.WRAP_CONTENT,
                        ConstraintLayout.LayoutParams.WRAP_CONTENT
                    )
